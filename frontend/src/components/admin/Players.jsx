...
const editPlayerPop = async (id) => {
  ...
  setPlayerId(id)
  try {
    const res = await getPlayer(id).unwrap()
    setPlayerName(res)
  } catch (error) {
    console.log(error)
  }
...
const memoPlayers = useMemo(() => {
 ...
 if(isLoading) {
  ...
  return (
  <Container>
    {memoPlayers.map(x => <div className="teams p-2" key={x._id}>
      <div className="team-name">{x.appName}</div>
      <div>{x.nowCost.toFixed(1)}</div>
          <div><Button onClick={() =>editPlayerPop(x._id)} className="btn btn-warning">Edit</Button></div>
          <div><Button onClick={() => deletePlayerPop(x._id)} className="btn btn-danger">Delete</Button></div>
      </div>)}
      <div className="add-button p-2">
        <Button onClick={addPlayerPop}  className="btn btn-success">Add Player</Button>
      </div>
      <AddModal
      submit={submit}
