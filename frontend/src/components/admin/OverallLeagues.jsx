...
const OverallLeagues = () => {
  ...
  const [updateOverallTable, { isLoading: a }] =
    useUpdateOverallTableMutation();
  ...
  const deleteOverallLeagueNow = async () => {
    try {
      const res = await deleteLeague(overallLeagueId).unwrap();
  ...
  return (
    <Container className="p-2">
        ...
        <>
          <div className="overflow-auto">
            <table className="border rounded-lg">
              ...
              <tbody>
                {formattedDetails?.map((x, idx) => (
                  <tr
                    className={`${idx % 2 === 1 ? "bg-green-200" : "bg-white"} border border-gray-400 font-semibold`}
                    key={x._id}
                  >
                    ...
                    <td className="px-4 py-2 text-center">
                      <span>{x?.updatedDate}</span>,&nbsp;
                      <span>{x?.updatedTime}</span>
                    </td>
      ...
      <div className="add-button p-2">
        ...
        {!!memoOverallLeagues?.length && (
          <Button onClick={() => updateOverallLeague()}>
