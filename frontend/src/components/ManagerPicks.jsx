...
const ManagerPicks = (props) => {
  ...
  const defenders = picks
    ?.filter((pick) => pick?.playerPosition === 2 && pick?.multiplier > 0)
    ?.sort((a, b) => (a.slot > b.slot ? 1 : -1));
  const midfielders = picks
    ?.filter((pick) => pick?.playerPosition === 3 && pick?.multiplier > 0)
    ?.sort((a, b) => (a.slot > b.slot ? 1 : -1));
  const forwards = picks
    ?.filter((pick) => pick?.playerPosition === 4 && pick?.multiplier > 0)
    ?.sort((a, b) => (a.slot > b.slot ? 1 : -1));
  const bench = picks
  ...
  return (
    <div>
      {isMatchday ? (
        <div style={{ height: "100px" }} className="spinner">
          <Spinner />
        </div>
      ) : (
