...
if(isLoading) {
  ...
  return (
    <div className="fix-body">
      ...
      onClick={onDecrement}
      className= {`${page === +1 && 'btn-hide'} btn btn-controls`}
      id="prevButton"
    >
      <BsChevronLeft />
    </button>
      ...
      onClick={onIncrement}
      className={`${page === fixtures?.length && 'btn-hide'} btn btn-controls`}
      id="nextButton"
    >
      <BsChevronRight />
    </button>
    </section>
      {copy?.filter(x => +(x?._id?.id) === +page)?.map((fixture) => (
        <div key={fixture?._id?._id}>
          ...
          <div>
            {fixture?.fixtures?.map((x, idx) => (
              <div key={x._id}>
            <div className="deadline">{returnDay(fixture?.fixtures, idx)}</div> 
                <div
                  onClick={onClick}
                  className={`${stats && "bg-teams"} teams`}
                >
                  <div className="home">
                    <div className="team">
                    ...
                    {x?.stats?.length > 0 ?
                            x?.stats?.filter(x => x.identifier === 'goalsScored')[0]
                                .home.map(x => x.value).reduce((a, b) => a + b, 0) + x?.stats?.filter(x => x.identifier === 'ownGoals')[0]
                                    .away.map(x => x.value).reduce((a, b) => a + b, 0) :
                            getPmString(new Date(getTime(x?.kickOffTime)).toLocaleTimeString())}
                    </div>
                  </div>
                  <div className="away">
                  <div className={`${x?.stats?.length > 0 ? 'score' : 'time-2'}`}>
                    {x?.stats?.length > 0 ?
                            x?.stats?.filter(x => x.identifier === 'goalsScored')[0]
                                .home.map(x => x.value).reduce((a, b) => a + b, 0) + x?.stats?.filter(x => x.identifier === 'ownGoals')[0]
                                    .away.map(x => x.value).reduce((a, b) => a + b, 0) :
                            getPm(new Date(getTime(x?.kickOffTime)).toLocaleTimeString())}
                    </div>
                      <div className="ticker-image"></div>
                    <div className="team">
