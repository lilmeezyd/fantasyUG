...
import { useGetWeeklyStandingsQuery, useGetLeagueQuery } from "../slices/leagueApiSlice";
...
const WeeklyLeague = () => {
  ...
  const { data = [], isLoading } = useGetWeeklyStandingsQuery({id, mid, page, limit});
  ...
  const start = leagueDetails?.startMatchday;
  const end = currentId;
  const options = [];
  ...
  const onChange = (e) => {
    ...
    navigate(`/leagues/${id}/standings/matchday/${e.target.value}`)
  };
    ...
    <>
      {<div>
            ...
            <div className="text-3xl text-center p-2 font-semibold">
              {leagueDetails?.name}&nbsp;&nbsp;Weekly Table
            </div>
            <div className="text-xl text-center p-2 font-semibold">Matchday {mid}</div>
            <div className="bg-gray-200 h-0.5 w-[60%] m-auto"></div>
            <div className="standing-header sort-options">
              <label htmlFor="sort_by">Filter By:</label>
              <div>
                <select
                  onChange={onChange}
                                ...
                                {data?.standings?.map((entrant, idx) => (
                                  ...
                                  >
                                    ...
                                    <td className="px-4 py-3 text-center text-truncate">
                                      <div>{entrant?.matchdayPoints}</div>
                                    </td>
                                  </tr>
                                ))}
                              </tbody>
      ...
      {<div>
        {data?.entrants?.length > 0 && (
          <>
            <div className="text-xl text-center p-2 mt-2 font-semibold">
