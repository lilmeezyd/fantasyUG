                  ...
                  <div className="view">
                    ...
                    >
                      ...
                      <optgroup label="By Team">
                        {teams?.filter(x => x.name !== 'Neutral')?.map((team, idx) => {
  ...
  { goalkeepers.length > 0 ? (<div className='table-one' id='goalkeepers'>
      ...
      <div>
          {goalkeepers.map((goalkeeper) => {
              let teamObj = teams?.find(x => x._id === goalkeeper.playerTeam)
  ...
  <div className='player-header-1'>
      ...
      <div>
          {defenders.map((defender) => {
              let teamObj = teams?.find(x => x._id === defender.playerTeam)
              let short_name = teamObj?.shortName
              let positionObj = elementTypes?.find(x => x._id === defender.playerPosition)
              let short_pos = positionObj?.shortName
              let forwardImage = positionObj?.code === 1 ? `${teamObj?.code}_1-66`:
            `${teamObj?.code}-66`   
                  return (<PlayerCard 
  ...
  <div className='player-header-1'>
      ...
      <div>
          {midfielders.map((midfielder) => {
              let teamObj = teams?.find(x => x._id === midfielder.playerTeam)
              let short_name = teamObj?.shortName
              let positionObj = elementTypes?.find(x => x._id === midfielder.playerPosition)
              let short_pos = positionObj?.shortName
              let forwardImage = positionObj?.code === 1 ? `${teamObj?.code}_1-66`:
            `${teamObj?.code}-66`    
                  return (<PlayerCard 
  ...
  <div className='player-header-1'>
      ...
      <div>
          {forwards.map((forward) => {
              let teamObj = teams?.find(x => x._id === forward.playerTeam)
              let short_name = teamObj?.shortName
