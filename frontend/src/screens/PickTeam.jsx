...
const PickTeam = () => {
  ...
  const reducer = (state, action) => {
    ...
    if (action.type === "GKP_SET_SWITCH") {
      ...
      const okayed = state.picks
        .filter((x) => x._id !== data._id && x.playerPosition === 1)
        .map((x) => x.slot);
      const blocked = state.picks
        .filter((x) => x.playerPosition !== 1)
        .map((x) => x.slot);
      return {
        ...state,
        switcher: data,
    ...
    if (action.type === "DEF_SET_SWITCH") {
      ...
      if (data.multiplier === 0) {
        const okayB = state.picks
          .filter((x) => x.multiplier === 0 && x.slot > 12)
          .map((x) => x.slot);
        if (state.FWD === 1) {
          const okayS = state.picks
            .filter((x) => x.playerPosition !== 1 && x.playerPosition !== 4)
            .map((x) => x.slot);
        ...
        if (state.DEF === 3) {
          const okayS = state.picks
            .filter((x) => x.slot > 12 && x.playerPosition === 2)
            .map((x) => x.slot);
          const okayed = [...okayS];
          const blocked = ids.filter((x) => !okayed.includes(x));
          return {
    ...
    if (action.type === "MID_SET_SWITCH") {
      ...
      if (data.multiplier === 0) {
        ...
        if (state.DEF === 3) {
          const okayS = state.picks
            .filter((x) => x.playerPosition !== 1 && x.playerPosition !== 2)
            .map((x) => x.slot);
    ...
    if (action.type === "FWD_SET_SWITCH") {
      ...
      if (data.multiplier === 0) {
        ...
        if (state.FWD === 1) {
          const okayS = state.picks
            .filter((x) => x.slot > 12 && x.playerPosition === 4)
            .map((x) => x.slot);
          const okayed = [...okayS];
          const blocked = ids.filter((x) => !okayed.includes(x));
          return {
  ...
  useEffect(() => {
    ...
    const defenders = managerPicks?.picks?.filter(
      (pick) => pick?.playerPosition === 2 && pick?.multiplier > 0
    )?.length;
    const midfielders = managerPicks?.picks?.filter(
      (pick) => pick?.playerPosition === 3 && pick?.multiplier > 0
    )?.length;
